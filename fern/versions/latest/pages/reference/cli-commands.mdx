...
gym list environments [<name>] [--json]       # list environments (or inspect one)
...
gym search [<type>] <query> [--json]          # filter a component type (default: benchmarks) by query
...
List the environments available under `environments/`, by short name (with their domain and description).
The names map to environments' configs and can be passed to `--environment` on commands like `gym env start` / `gym eval run`.

| Option | Description |
| --- | --- |
| `[<name>]` | Inspect a single environment (config path, resources servers, agent, datasets). |
...
| Option | Description |
| --- | --- |
| `[<name>]` | Inspect a single model (its path and available `--model-type` values). Accepts a `<model>/<flavor>` token. |
...
Filter a component type's listing to fuzzy matches on a query (against each component's name/value and domain, plus type-specific fields).
The type is optional and defaults to `benchmarks`.

| Option | Description |
| --- | --- |
| `[<type>]` | Component type to search: `benchmarks` (default), `environments`, `agents`, `models`, or `resources-servers`. |
| `QUERY` | Query to match (positional, required). |
| `--json` | Output matches as JSON. |

```bash
gym search math                       # benchmarks matching "math" (default type)
